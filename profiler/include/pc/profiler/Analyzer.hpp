...
namespace pc
{
   namespace profiler
   {
      ...
      struct Analyzer
      {
         ...
         Analyzer(Numeric value = 0, std::size_t count = 0) :
             average(value), low(DBL_MAX), high(DBL_MIN), count(count),
             NoOfClosesToHigh(0)
         {
         }
         Analyzer& operator+=(Numeric const incBy)
         {
...
#ifdef PC_PROFILE
         friend std::ostream& operator<<(std::ostream& os, Analyzer<Numeric> const& value)
         {
         ...
         Analyzer<Numeric>& operator+=(timespec time)
         {
